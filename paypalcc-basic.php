...
class ppcc {
    ...
    protected $data = array(
		...
		'time_stamp' => 0,
		'exrlog' => 'on',
		'api_selection' => 'custom',
		'retrieval_count' => 0,
		'autocomplete' => 'off', //PayPal auto completion for virtual products  //was 'off'
		'autoprocessing' => 'off', //PayPal auto processing for standard products //was 'off'
		...
		'handling_title' => 'Handling', //handling custom title?
		'lower_treshold' =>'off',
		'lower_treshold_amount'  =>'0.0',
		'upper_treshold' =>'off',
		'upper_treshold_amount'  =>'0.0'
    );
    ...
    public function init() {
		...
		//new method for converted totals in payment description
		add_filter('woocommerce_update_order_review_fragments', array($this, 'ppcc_checkout_payment_fragment')); //PPEXP gateway
		...
		// enque ppcc_checkout.js
		add_action('woocommerce_review_order_before_payment', array($this, 'ppcc_enque_ppcc_checkout'), 10, 1);
	
		add_action('woocommerce_review_order_before_payment', array($this, 'ppcc_calculate_totals'), 10, 1);
		
		// handling fee
		add_action( 'woocommerce_cart_calculate_fees', array( $this, 'ppcc_calculate_totals' ), 10, 1 );

		if(1==2){
			add_action( 'before_woocommerce_pay', array( $this, 'ppcc_checkout_payment_fragment' ), 10, 1 );	
		}
	...
	public function ppcc_enque_ppcc_checkout() {
			
		$ppcc_plugin_file =  __FILE__ ;
		$ppcc_data=get_plugin_data( $ppcc_plugin_file);
	
		wp_register_script( 'ppcc_checkout', plugins_url( '/assets/js/ppcc_checkout.js', __FILE__ ),'woocommerce.min.js', $ppcc_data['Version'], true);//pass variables to javascript
	...
	//new method to deliver the converted amount to the payment fragments
	public function ppcc_checkout_payment_fragment($ajax){
		global $woocommerce;
		...
		$order_total_inc_tax = number_format($woocommerce->cart->total  * $options['conversion_rate'], 2, '.', '' )." ".$options['target_currency'];
		...
		$handling_amount_total = number_format( $woocommerce->cart->fee_total * $options['conversion_rate'], 2, '.', '' )." ".$options['target_currency'];
		...
		$data = array(	
						...
						'handling_amount_total'=>$handling_amount_total,	
		...
		foreach ($data as $key => $value) {
			...
			//$re = '/<span class=\\\"ppcc_'.$key.'l\\\"><\\\\\/span>/m';
			//preg_replace($re, '<span class=\\\"ppcc_'.$key.'\\\">'.$value.'<\\\\\/span>', $ajax['.woocommerce-checkout-payment']);
			
			$html .= '<span class="shadowppcc_'.$key.'">'.$value.'</span>';
			//$ajax['.test'] .=  ' # '.$re.'->'.$value;
		}

		//APPEND a div with the "shadowed" span tags which will be removed by the JS after updating the span tags in the payment description
		$ajax['.woocommerce-checkout-payment'] .= '<div id="PPCC-checkout-payment" style="display: none;">'.$html.'</div>';
	...
	public function ppcc_add_order_notes_to_email($order,$is_admin) {
	    ...
	    if ( 'paypal' == $order->payment_method ) {
		    ...
		    global $woocommerce;
		    echo sprintf('<li>'.$options['order_email_note'].'</li>',$options['conversion_rate'], $options['target_currency'], $order->get_order_currency(), wc_price($converted_total,array('currency'=> $options['target_currency'])),$options['handling_percentage'].'%',$handling_amount_converted);
			echo '</ul>';
...
die;*/
    ...
    public function options_do_page() {
		...
		//warning for inproper treshold setting
		if($options['upper_treshold_amount'] < $options['lower_treshold_amount'] && $options['lower_treshold']=='on' && $options['upper_treshold']=='on') {
			echo '<div class="error"><p>'.__('When using lower and upper handling fee tresholds, the upper treshold amount needs to be higher then the lower treshold amount.').'</p></div>';
		} 

		if (null == ($woocommerce->payment_gateways->get_available_payment_gateways()))
		{
			echo '<div class="error"><p>'.__("Don't forget to select a Payment Gateway in WooCoomerce settings.").'</p></div>';
		}
		...
		if ($_GET['page']=='ppcc_options'){
			...
			//update_paypal_description(); //deprecated
		...
		($options['lower_treshold']=="on"?$lower_treshold_checkbox_status='checked="checked"': $lower_treshold_checkbox_status='');
		($options['upper_treshold']=="on"?$upper_treshold_checkbox_status='checked="checked"': $upper_treshold_checkbox_status='');
		...
		$handling_amount_converted = wc_price(number_format( (float)$options['handling_amount'] * (float)$options['conversion_rate'], 2, '.', '' ),array('currency'=> $options['target_currency']));
		...
		$GatewayDescription= 'Cart Total: <span class="ppcc_cart_total"></span><br>Shipping Total: <span class="ppcc_shipping_total"></span><br>Handling <span class="ppcc_handling_percentage"></span>% plus <span class="ppcc_handling_amount"></span> fixed.<br>Handling total: <span class="ppcc_handling_amount_total"></span><br>Order Total Tax: <span class="ppcc_tax_total"></span><br>Order Total inclusive Tax: <span class="ppcc_total_order_inc_tax"></span><br>Conversion Rate: <span class="ppcc_cr"></span>';
...
?>
			<h2 class="nav-tab-wrapper">
			    ...
			    <a href="#?section=HandlingFee" class="nav-tab" onclick="
			    	...
			    	jQuery('.nav-tab').removeClass('nav-tab-active');
				    jQuery(this).addClass('nav-tab-active');
				    "><?php echo __('Handling','PPCC-PRO')?></a>
...
<?php
		echo '   
				...
				<table class="form-table">
	 				<tbody>
						<tr valign="top">
							...
							<td class="forminp"><input disabled type="checkbox" id="ucc" value="1" name="'. $this->option_name.'[ppcc_use_custom_currency]" '.$custom_currency_checkbox_status.' />
								<span id="customcurrency" '.$hidden.'>
									Code:<span class="woocommerce-help-tip" data-tip="'. __('Your custom currency ISO-4217-Code. If the conversion fails...make sure you are using the correct code','PPCC-PRO').'"></span> <input disabled type="text" size="3" id="ccc" name="'. $this->option_name.'[ppcc_custom_currency_code]" value="'.$options['ppcc_custom_currency_code'].'" '.$required.'/>
									Symbol:<span class="woocommerce-help-tip" data-tip="'. __('Your custom currency symbol.','PPCC-PRO').'"></span> <input disabled type="text" size="3" id="ccs" name="'. $this->option_name.'[ppcc_custom_currency_symbol]" value="'.$options['ppcc_custom_currency_symbol'].'" '.$required.'/>
									Name:<span class="woocommerce-help-tip" data-tip="'. __('Your custom currency name.','PPCC-PRO').'"></span> <input disabled type="text" size="8" id="ccn" name="'. $this->option_name.'[ppcc_custom_currency_name]" value="'.$options['ppcc_custom_currency_name'].'" '.$required.'/>
								</span>	
							</td>
	                    </tr>
	                   <tr valign="top">
	                        ...
	                        <td class="forminp"><input disabled type="text" size="3" value="'. get_woocommerce_currency().'"  disabled/><label for="ppcc_source_cur"> (convert from currency, this is your WooCommerce Shop Currency)</label></td>
	                    </tr>
	                    <tr valign="top">
						<th class="titledesc" scope="row">
	                    ...
	                    <tr valign="top">
							...
							<td class="forminp" >
								...
								'. __('press this button','PPCC-PRO').'&#9658;<input disabled type="button" class="button-primary" id="selected_currency" value="'.$exrdata.'"/> '. __('to accept the current exchange rate.','PPCC-PRO').'<span class="woocommerce-help-tip" data-tip="Currently selected provider\'s internal Id = '.$options['api_selection'].'"></span>
							</td>
	                    </tr>	                    
					</tbody>
				</table>
			...
			<div class="ppcc_tab" id ="ExhangeRateProvider" style="display: none;">
				...
				<tbody>
					...
					<tr valign="top">
						...
						<td class="forminp">
							<input disabled type="radio" id="currencyconverterapi" name="'. $this->option_name.'[api_selection]" value="currencyconverterapi" '.$currencyconverterapi_checked.'/>
							<input disabled type="text" size="35" id="currencyconverterapi_id" name="'. $this->option_name.'[currencyconverterapi_id]" value="available in Pro Version"/>
							<span class="woocommerce-help-tip" data-tip="'. __('Register your free.currencyconverterapi API ID in this text field and save.<br/>(Base = EUR)','PPCC-PRO').'"></span> Get the API ID here: <a href="https://free.currencyconverterapi.com/">free.currencyconverterapi.com</a>
							<div style="display: none;"><br>Test the API ID here: <a href="https://free.currencyconverterapi.com/api/v6/convert?apiKey='.$options['currencyconverterapi_id'] .'&q=' . get_woocommerce_currency() . '_' . $options['target_currency'] . '&compact=y">https://free.currencyconverterapi.com/api/v6/convert?apiKey='.$options['currencyconverterapi_id'] .'&q=' . get_woocommerce_currency() . '_' . $options['target_currency']. '&compact=y</a>
							</div>								
						</td>						
					</tr>
					...
					<tr valign="top" '.$hide_bnro.'>
						...
						<td class="forminp">
							<input disabled type="radio" id="bnro" name="'. $this->option_name.'[api_selection]" value="bnro" '.$bnro_checked.'/><span class="woocommerce-help-tip" data-tip="'. __('BANK of Romania<br>(Base = RON)','PPCC-PRO').'"></span> Source <a href="http://www.bnro.ro/nbrfxrates.xml">BNRO</a>						
						</td>
                    </tr>
					...
					<tr valign="top">
						...
						<td class="forminp">
							<input disabled type="radio" id="fixer_io_api_id" name="'. $this->option_name.'[api_selection]" value="fixer_io_api_id" '.$api_fixer_io_checked.'/>
							<input disabled type="text" size="35" id="fixer_io_api_id" name="'. $this->option_name.'[fixer_io_api_id]" value="available in Pro Version"/>
					...
					<tr valign="top">
						...
						<td class="forminp">
							<input disabled type="radio" id="oer_api_id" name="'. $this->option_name.'[api_selection]" value="oer_api_id" '.$oer_api_checked.'/> 
							<input disabled type="text" size="35" id="oer_api_id" name="'. $this->option_name.'[oer_api_id]" value="available in Pro Version"/>
					...
					<tr valign="top">
						...
						<td class="forminp">
							<input disabled type="radio" id="xignite_api" name="'. $this->option_name.'[api_selection]" value="xignite" '.$xignite_checked.'/> 
							<input disabled type="text" size="35" id="oer_api_id" name="'. $this->option_name.'[xignite_api_id]" value="available in Pro Version"/>
					...
					<tr valign="top">
						...
						<td class="forminp">
							<input disabled type="radio" id="apilayer_api" name="'. $this->option_name.'[api_selection]" value="apilayer" '.$apilayer_checked.'/> 
							<input disabled type="text" size="35" id="oer_api_id" name="'. $this->option_name.'[apilayer_api_id]" value="available in Pro Version"/>
			...
			<div class="ppcc_tab" id ="HandlingFee" style="display: none;">
				<h2>'. __('Add (optional) handling fees/bonuses to the transaction.','PPCC-PRO').'
				</h2>
				<table class="form-table">
				<tbody>
					<tr valign="top">
						<th class="titledesc" scope="row" style="width: 250px;">
							...
							<span class="woocommerce-help-tip" data-tip="'. __('Enter a percentage value other then 0 to add or subtract a percentage on the total order amount(before fixed amount is added). Negative values are bonuses, positive values will be fees.','PPCC-PRO').'"></span>
						</th>
						<td class="forminp">
							<input style ="width: 5em;" type="number" placeholder="1.0" step="0.01" min="-10" max="10" id="handling_percentage"  name="'. $this->option_name.'[handling_percentage]" value="'.$options['handling_percentage'].'" /> in %
							<span class="woocommerce-help-tip" data-tip="'. __('A handling fee or bonus percentage should be in a reasonable range, certainly and most probably lower then 5%. Min value = -10.00%, max value = 10.00% (0 equals no percentual handling fee).','PPCC-PRO').'"></span> 
						</td>
                    </tr>
                    <tr valign="top">
						<th class="titledesc" scope="row" style="width: 250px;">
							<label >'. __('Fixed amount','PPCC-PRO').' </label>
							<span class="woocommerce-help-tip" data-tip="'. __('Enter a handling fee or bonus amount value other then 0 to add or subtract a fixed amount to the order total as handling fee/bonus. Negative values are bonuses, positive values will be fees.','PPCC-PRO').'"></span>
						</th>
						<td class="forminp">
							<input style ="width: 5em;" type="number" placeholder="1.0" step="0.01" min="-10000" max="10000" id="handling_amount"  name="'. $this->option_name.'[handling_amount]" value="'.$options['handling_amount'].'" /> in '.get_woocommerce_currency().'
							<span class="woocommerce-help-tip" data-tip="'. __('A handling fee amount should be in a reasonable range given in the shop currency. Min value = 0.00(equals no fixed handling amount), max value = 10\'000.00 units in shop currency.','PPCC-PRO').'"></span>
						</td>
                    </tr>
                    <tr valign="top">
						<th class="titledesc" scope="row" style="width: 250px;">
						...
						<td class="forminp"><input disabled type="checkbox" id="htax" value="1" name="'. $this->option_name.'[handling_taxable]" '.$handling_taxable_checkbox_status.' /> Handling will be taxed when checked.
						</td>
                    </tr> 
                    <tr valign="top">
						<th class="titledesc" scope="row" style="width: 250px;">
						...
						<td class="forminp"><input disabled type="checkbox" id="htax" value="1" name="'. $this->option_name.'[shipping_handling_fee]" '.$shipping_handling_fee_checkbox_status.' /> Handling percentage will also take shipping cost into account.
						</td>
                    </tr>                  
                    <tr valign="top">
						<th class="titledesc" scope="row" style="width: 250px;">
							<label >'. __('Custom title','PPCC-PRO').' </label>
							<span class="woocommerce-help-tip" data-tip="'. __("Name your handling . 'Handling ' recommended.",'PPCC-PRO').'"></span>
						</th>
						<td class="forminp"><input disabled type="text" id="htitle" value="available in Pro Version" name="'. $this->option_name.'[handling_title]" /> Custom handling title.
						</td>
                    </tr> 
                    <tr valign="top">
						<th class="titledesc" scope="row" style="width: 250px;">
							<label >'. __('Lower treshold','PPCC-PRO').' </label>
							<span class="woocommerce-help-tip" data-tip="'. __('Check this to set a lower treshold when handling shall be added. (Charges handling only if the order is below some total sum e.g. if the orders is below $20 without shipping, charge $2 handling fee. If the order is * above $20, do not charge any additional fee.)','PPCC-PRO').'"></span>
						</th>
						<td class="forminp">
						<input disabled type="checkbox" id="ltcs" value="1" name="'. $this->option_name.'[lower_treshold]" '.$lower_treshold_checkbox_status.' /> Handling will be added if order total is below lower treshold of:

							<input style ="width: 5em;" type="number" placeholder="0.0" step="0.01" min="0" max="'.$options['upper_treshold_amount'].'" id="lower_treshold_amount"  name="'. $this->option_name.'[lower_treshold_amount]" value="'.$options['lower_treshold_amount'].'" /> in '.get_woocommerce_currency().'
							<span class="woocommerce-help-tip" data-tip="'. __('A lower treshold amount should be in a reasonable range given in the shop currency. Min value = 0.00, max value = 10\'000.00 units in shop currency.','PPCC-PRO').'"></span>
						</td>
                    </tr>  
                    <tr valign="top">
						<th class="titledesc" scope="row" style="width: 250px;">
							<label >'. __('Upper treshold','PPCC-PRO').' </label>
							<span class="woocommerce-help-tip" data-tip="'. __('Check this to set an upper treshold when handling fees shall be added. Charge handling fee only if the order is above some total sum e.g. if the orders is above $20 without shipping, charge $2 handling fee. If the order is below $20, do not charge any additional fee.)','PPCC-PRO').'"></span>
						</th>
						<td class="forminp">
							<input disabled type="checkbox" id="utcs" value="1" name="'. $this->option_name.'[upper_treshold]" '.$upper_treshold_checkbox_status.' /> Handling will be added if order total is above upper treshold of:

							<input style ="width: 5em;" type="number" placeholder="0.0" step="0.01" min="'.$options['lower_treshold_amount'].'" max="10000" id="upper_treshold_amount"  name="'. $this->option_name.'[upper_treshold_amount]" value="'.$options['upper_treshold_amount'].'" /> in '.get_woocommerce_currency().'
							<span class="woocommerce-help-tip" data-tip="'. __('A upper tresold amount should be in a reasonable range given in the shop currency. Min value = 0.00, max value = 10\'000.00 units in shop currency.','PPCC-PRO').'"></span>
						</td>
                    </tr> 
   				</tbody>
				</table>
			...
			<div class="ppcc_tab" id ="OrderStatus" style="display: none;">
				...
				<tbody>
					<tr valign="top">
						...
						<td class="forminp"><input disabled type="checkbox" id="ac" value="1" name="'. $this->option_name.'[autocomplete]" '.$autocomplete_checkbox_status.' /> Orders for virtual (downloadable) products will be completed when checked and stock will be reduced.
						</td>
                    </tr>
                    <tr valign="top">
						<th class="titledesc" scope="row" style="width: 250px;">
						...
						<td class="forminp"><input disabled type="checkbox" id="ap" value="1" name="'. $this->option_name.'[autoprocessing]" '.$autoprocessing_checkbox_status.' /> Orders for standard products will be set to processing and stock will be reduced when checked.
						</td>
                    </tr>
				</tbody>
				</table>
				...
				<table class="form-table">
					<tbody>
						<tr valign="top">
							...
							<td class="forminp"><input disabled type="checkbox" id="ac" value="1" name="'. $this->option_name.'[email_order_completed_note]" '.$email_order_completed_note_checkbox_status.' /> Adds a note to the order receipt email about the conversion and converted total amount paid via PayPal.
							</td>
	                    </tr>
	                    <tr>
	                    <th>
	                    ...
	                    <td>
	                    	<textarea disabled cols="80" rows="3" id="order_email_note" name="'. $this->option_name.'[order_email_note]" />available in Pro Version</textarea>
	                    </td>
	                    </tr>
						<tr valign="top">
	                    <th>
	                    ...
	                    <td><em>'.
	                    	sprintf($options['order_email_note'],$options['conversion_rate'], $options['target_currency'], get_woocommerce_currency(), wc_price('100',array('currency'=> $options['target_currency'])),$options['handling_percentage'].'%',$handling_amount_converted)
	                    .'</em></td>
	                    </tr>
					</tbody>
				</table>
				...
				<table class="form-table">
					<tbody>
	                    <tr valign="top">
							...
							<td class="forminp"><input disabled type="checkbox" id="ud" value="1" name="'. $this->option_name.'[auto_update]" '.$auto_update_checkbox_status.' /> '. __('Make sure to have a valid API ID registered above when using Open Exchange Rates API!','PPCC-PRO').'
							</td>
	                    </tr>
		               <tr valign="top">
							<th class="titledesc" scope="row">
							...
							<td class="forminp">
								<input disabled type="text" id="ludisabled" value="'.date( 'Y-m-d H:i:s',(int)$options['time_stamp']).'" disabled />
								<input disabled type="text" id="lu" name="'. $this->option_name.'[time_stamp]" value="'.(int)$options['time_stamp'].'" hidden />
							</td>
	                    </tr>
						<tr valign="top">
							<th class="titledesc" scope="row">
							...
							<td class="forminp"><input disabled type="checkbox" id="ud" value="1" name="'. $this->option_name.'[exrlog]" '.$exrlog_checkbox_status.' /> '.__('Notification email for each Exchange Rate Update will be sent to ','PPCC-PRO').get_ppcc_option('admin_email').'
							</td>
	                    </tr>					
						<tr valign="top">
						...
						<tr valign="top">
							...
							<td class="forminp">	
								<code>' . site_url() .'/wp-admin/admin-ajax.php?action=ppcc&ppcc_function=(available in Pro Version)</code><br/><a href="https://www.easycron.com?ref=14909" title="EAYSYCRON.COM"><img src="'.plugins_url( '/assets/images/easycron-logo.png',__file__).'" alt"easycron.com"></a>...provides a hassle free solution !
							</td>
						</tr>
						<tr valign="top">
							<th scope="row" class="titledesc">
								<label >'.__('Hook to call for "WP Cron" Job','PPCC-PRO').' </label>
									<span class="woocommerce-help-tip" data-tip="'.__('Call the Hook \'available in Pro Version\' from your Cron Job Plugin. But consider that Wordpress\'s Cron System is dependant on traffic and not necessarily timely exact.','PPCC-PRO').'" />
							</th>
							<td class="forminp">
								<code>available in Pro Version</code>
							</td>
						</tr>
	                </tbody>
           		</table>
			...
			<div class="ppcc_tab" id="info" style="display: none;">
                ...
                <tr valign="top">
					...
					<td class="forminp">
						<input disabled type="text" size="5" id="rcdisabled" value="'.$options['retrieval_count'].'"  disabled/>
						<input disabled type="text" size="5" id="rc" name="'. $this->option_name.'[retrieval_count]" value="'.$options['retrieval_count'].'" hidden/>
					</td>	
                </tr>
				<tr valign="top">
	...
	public function validate($input) {
		...
		// $valid['currencyconverterapi_id'] = $input['currencyconverterapi_id'];
		...
		// ($input['lower_treshold']=="1"||$input['lower_treshold']=="on")? $valid['lower_treshold'] = "on":$valid['lower_treshold'] = "off";
		// $valid['lower_treshold_amount'] = $input['lower_treshold_amount'];

		// ($input['upper_treshold']=="1"||$input['upper_treshold']=="on")? $valid['upper_treshold'] = "on":$valid['upper_treshold'] = "off";
		// $valid['upper_treshold_amount'] = $input['upper_treshold_amount'];
	...
	public function ppcc_calculate_totals($totals){
		...
		if (strpos(strtolower($current_gateway->title), 'paypal')!== false)
			...
			// reasonable values for handling fee are mandatory
			if ($options['handling_percentage']!= 0 || $options['handling_amount']!= 0)
				{
					...
					if ($options['handling_percentage']!= 0 && $options['handling_amount']!= 0)//both percentage and fixed amount
					{
					...
					elseif ($options['handling_percentage']!= 0) {//percentage only
						$handling_fee_title = $options['handling_percentage'].'% '. $options['handling_title'];
					}
					else//fixed amount only
					{
						$handling_fee_title =  $options['handling_title'];
					}
					//check trescholds
					/*
					* Charge handling fee only if the order is below some total sum e.g. if the orders is below $20 without shipping, charge $2 handling fee. If the order is * above $20, do not charge any additional fee.

					* Or charge handling fee only if the order is above certain amount e.g. if the order is below $20 without shipping do not charge handling fee and if the 
					* order is above $20 charge $2 handling fee.
					*/

					if(($options['lower_treshold']=='on' && ($options['lower_treshold_amount'] > 0) && ($totals->cart_contents_total < $options['lower_treshold_amount'])) || ($options['upper_treshold']=='on' && $options['upper_treshold_amount'] > 0 && $totals->cart_contents_total > $options['upper_treshold_amount'])){
						//tresholds checked, greater zero, and cart total outside lower and upper treshold
						if($totals->cart_contents_total < $options['lower_treshold_amount']){
							$handling_fee_title .= '(cart total < '.$options['lower_treshold_amount'].')';
						}
						elseif($totals->cart_contents_total > $options['upper_treshold_amount']){
							$handling_fee_title .= '(cart total > '.$options['upper_treshold_amount'].')';
						}
						$woocommerce->cart->add_fee( $handling_fee_title , $handling_fee , ($options['handling_taxable'] == 'on'?true:false) , 'standard');
					}
					elseif ($options['lower_treshold']=='off' && $options['upper_treshold']=='off'){
						//treshold will be ignored
						$woocommerce->cart->add_fee( $handling_fee_title , $handling_fee , ($options['handling_taxable'] == 'on'?true:false) , 'standard');
					}				
